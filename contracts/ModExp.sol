...
/// @notice Addition chain for ^{(N-1)/2}
library ModExpLegendre {
    function run(uint256 t2) internal pure returns (uint256 t0) {
        assembly {
            let
                n
            ...
            t0 := mulmod(t2, t2, n)
            let t1 := mulmod(t0, t2, n)
            let t3 := mulmod(t1, t0, n)
            let t4 := mulmod(t1, t1, n)
            let t8 := mulmod(t3, t0, n)
            let t5 := mulmod(t4, t1, n)
            let t6 := mulmod(t4, t3, n)
            t0 := mulmod(t4, t4, n)
            let t7 := mulmod(t8, t4, n)
            t4 := mulmod(t5, t4, n)
            t0 := mulmod(t0, t0, n)
            t0 := mulmod(t0, t0, n)
            t0 := mulmod(t0, t0, n)
